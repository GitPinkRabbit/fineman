...
- 定义两点 $(u, v)$ 间的 **$\boldsymbol{h}$ 跳距离** $\operatorname{dist}^h(u \rightsquigarrow v)$ 为它们之间不超过 $h$ 跳的路径的最短路径长度。若不存在则定义为 $+\infty$。
- 简单性质：
  - $\operatorname{dist}^h(u \rightsquigarrow v) < \operatorname{dist}^{h'}(u \rightsquigarrow v)$（$h' < h$）。
  - 即 $\operatorname{dist}^h$ 随 $h$ 的增加而降低。
  - $\operatorname{dist}^h$ 不能为 $-\infty$（与之相对地，$\operatorname{dist}$ 可以为 $-\infty$，即当图中存在负环时）。
  - 若图中不存在负环，则 $\operatorname{dist}^h$ 随 $h$ 的增加而降低是有限度的，即当 $h$ 增加至两点间最短路的跳数后，$\operatorname{dist}^h$ 将不再变化。
- 对点集 $S, T$ 也可以定义 $\operatorname{dist}^h(S \rightsquigarrow v)$、$\operatorname{dist}^h(u \rightsquigarrow T)$ 和 $\operatorname{dist}^h(S \rightsquigarrow T)$。性质类似。

**$\boldsymbol{h}$ 跳相关**：称 $(u, v)$ 是 **$\boldsymbol{h}$ 跳相关**的，当：

- $\operatorname{dist}^h(u \rightsquigarrow v) < 0$ 或 $\operatorname{dist}^h(v \rightsquigarrow u) < 0$。
...
- 进一步，定义一个点集 $S$ 的**负 $\boldsymbol{h}$ 跳可达范围**为
  $$
  R^h(S) = \{ v \in V : \operatorname{dist}^h(S \rightsquigarrow v) < 0 \} \text{。}
  $$
- 简单性质：
...
- $\operatorname{thru}^\beta(u \rightsquigarrow x \rightsquigarrow v) = \operatorname{dist}^\beta(u \rightsquigarrow x) + \operatorname{dist}^\beta(x \rightsquigarrow v)$。
- 若 $\operatorname{thru}^\beta(u \rightsquigarrow x \rightsquigarrow v) < 0$，则称 $x$ 是 **$\boldsymbol{\beta}$ 介于** $u \rightsquigarrow v$ **之间**的。
- 称 $u \rightsquigarrow v$ 的 **$\boldsymbol{\beta}$ 间性** $\operatorname{BW}^\beta(u \rightsquigarrow v)$ 为 $\beta$ 介于 $u \rightsquigarrow v$ 之间的点数，即 $\operatorname{BW}^\beta(u \rightsquigarrow v) = \lvert \{ x \in V : \operatorname{thru}^\beta(u \rightsquigarrow x \rightsquigarrow v) < 0 \} \rvert$。

**负夹心面包**：定义**负夹心面包**为满足如下条件的三元组 $(x, U, y)$：

- $U \subseteq V^-$，$x, y \in V$。
- $\operatorname{dist}^1(x \rightsquigarrow u) < 0$ 对所有 $u \in U$。
- $\operatorname{dist}^1(u \rightsquigarrow y) < 0$ 对所有 $u \in U$。
...
- 新图中的最短路保持不变，即 $\operatorname{dist}_\varphi(u \rightsquigarrow v) = \operatorname{dist}(u \rightsquigarrow v) + \varphi(u) - \varphi(v)$，或等价地，$\operatorname{dist}(u \rightsquigarrow v) = \operatorname{dist}_\varphi(u \rightsquigarrow v) - \varphi(u) + \varphi(v)$。
...
- 现有一张图 $G = (V, E^+ \cup E^-, w)$，目标是构造一张图 $H = (V_H, E_H, w_H)$，使得 $V \subseteq V_H$ 且：
  - 在图 $H$ 中，$\operatorname{dist}_H(u \rightsquigarrow v) = \operatorname{dist}(u \rightsquigarrow v)$（对 $u, v \in V$），且一条 $G$ 中的 $h$ 跳路径对应一条 $H$ 中的 $\lceil h / r \rceil$ 跳路径。
...
- 一个一般的构造如下：
  ...
  - 仍然使用 Johnson 技巧，令点 $u_i$ 的势为 $\varphi(u_i) = \operatorname{dist}^i(V \rightsquigarrow u)$。
  - 边权为 $w_H(u_i, v_j) = w(u, v) + \operatorname{dist}^i(V \rightsquigarrow u) - \operatorname{dist}^j(V \rightsquigarrow v)$。
  - 则根据 $\operatorname{dist}^{i + [w(u, v) < 0]}(V \rightsquigarrow v) \le \operatorname{dist}^i(V \rightsquigarrow u) + w(u, v)$，除了 $(u_i, u_0)$ 这些边，其他边权都 $\ge 0$。
  - 注：由于 $\operatorname{dist}^0(V \rightsquigarrow u) = 0$ 恒成立，有 $\varphi$ 在第 $0$ 层上恒为 $0$，故不影响 $\operatorname{dist}_H(u \rightsquigarrow v) = \operatorname{dist}(u \rightsquigarrow v)$ 成立。
...
- 此时，在图 $G_N$ 中，如果一个点 $u$ 在 $r$ 遥远子图之外，即 $\operatorname{dist}_N^r(X \rightsquigarrow u) \ge 0$，则必有 $\operatorname{dist}_N^r(V \rightsquigarrow u) \ge 0$，因为从其他点（非负点）出发并不比从负点出发更优。又由于 $\operatorname{dist}_N^i(V \rightsquigarrow u)$ 必然 $\le 0$，因为空路径的权值为 $0$。我们有 $\operatorname{dist}_N^r(V \rightsquigarrow u) = 0$，因此每个 $\operatorname{dist}_N^i(V \rightsquigarrow u)$ 都 $= 0$（对每个 $0 \le i \le r$）。
- 这就是说，对在 $r$ 遥远子图之外的点 $u$，有：
  - $w_H(u_i, v_j) = w(u, v) - \operatorname{dist}_N^j(V \rightsquigarrow v)$；
  - $w_H(v_i, u_j) = w(v, u) + \operatorname{dist}_N^i(V \rightsquigarrow v)$。
...
- 构造上述 $r$ 跳削减图仍需要计算 $\operatorname{dist}^i(V \rightsquigarrow u)$（$0 \le i \le r$），故需要 $O(r (m + n \log n))$ 时间。
...
- 忽略其余负边，得到图 $G_N = (V, E^+ \cup N, w)$。
- 使用势函数 $\varphi(u) = \operatorname{dist}_N^1(V \rightsquigarrow u)$。
- 此时对负边 $(u, v)$ 有新边权 $w_\varphi(u, v) = w(u, v) + \operatorname{dist}_N^1(V \rightsquigarrow u) - \operatorname{dist}_N^1(V \rightsquigarrow v)$。
- 由于 $I$ 是 $1$ 跳无关的，可知 $\operatorname{dist}_N^1(I \rightsquigarrow u) \ge 0$，这说明 $\operatorname{dist}_N^1(V \rightsquigarrow u) = 0$。
- 而 $\operatorname{dist}_N^1(V \rightsquigarrow v) \le w(u, v)$，故新边权 $w_\varphi(u, v) \ge 0$。
- 对于非负边 $(u, v)$，由于 $\operatorname{dist}_N^1(V \rightsquigarrow v) \le \operatorname{dist}_N^1(V \rightsquigarrow u) + w(u, v)$，显然有新边权 $\ge 0$。
- 时间复杂度为 $O(m + n \log n)$。
...
现在的目标是，固定常数 $\beta$ 和 $\tau$，并找到一个势函数使得 $\operatorname{BW}^\beta(u \rightsquigarrow v) \le n / \tau$ 对所有 $u \rightsquigarrow v$ 都成立。
...
- 注意 $\operatorname{thru}_\varphi^\beta(u \rightsquigarrow x \rightsquigarrow v) = \operatorname{thru}^\beta(u \rightsquigarrow x \rightsquigarrow v) + \varphi(u) - \varphi(v)$，不取决于 $\varphi(x)$。
- 故如果对某个点 $y$ 来说有 $\operatorname{thru}_\varphi^\beta < 0$，则考虑原 $\operatorname{thru}^\beta$，它必然是所有 $\operatorname{thru}^\beta$ 中比较小的。
- 由于特殊点是随机选取的，而 $\operatorname{thru}^\beta$ 只要比某个特殊点 $x$ 的 $\operatorname{thru}^\beta$ 大，就有 $\operatorname{thru}_\varphi^\beta \ge 0$。
- 对于一个特定的 $u \rightsquigarrow v$，$\operatorname{BW}^\beta(u \rightsquigarrow v) > n / \tau$ 的概率有上界：
...
- 现在的问题是，如何找到这样的势函数。（使得所有从特殊点出发或到达特殊点的 $\beta$ 跳距离都非负。）
  - 设特殊点集为 $T$。
  - 对每个 $x \in T$，求所有 $v \in V$ 的 $\operatorname{dist}^\beta(x \rightsquigarrow v)$ 和 $\operatorname{dist}^\beta(v \rightsquigarrow x)$。这一步需要 $O(\beta \lvert T \rvert (m + n \log n))$ 时间。
  - 建新图 $H = (V, E_H, w_H)$，其中 $E_H$ 包含有 $\lvert E^+ \rvert + 2 \lvert T \rvert n$ 条边，具体地：
    - 包含原图的所有非负边。（**注：原文不含此步。**）
    - 每对 $x \to v$ 和 $v \to x$ 都有一条边，边权相应地为 $\operatorname{dist}^\beta(x \rightsquigarrow v)$ 和 $\operatorname{dist}^\beta(v \rightsquigarrow x)$。
...
- 对一个负夹心面包 $(x, U, y)$ 和一个跳数 $\beta \ge 1$，考虑使用势函数
  $$
  \varphi(v) = \min(0, \max(\operatorname{dist}^\beta(x \rightsquigarrow v), -\operatorname{dist}^\beta(v \rightsquigarrow y))) \text{。}
  $$
  这个势函数在 $O(\beta (m + n \log n))$ 时间内可以计算出来。
- 这样的势函数满足：
  - 它是合法的，即不会把非负边转为负边。
    - 只需证明，对于非负边 $(u, v)$，有 $\varphi(v) \le \varphi(u) + w(u, v)$。
    - 由 $\operatorname{dist}^\beta(x \rightsquigarrow v) \le \operatorname{dist}^\beta(x \rightsquigarrow u) + w(u, v)$ 和 $\operatorname{dist}^\beta(u \rightsquigarrow y) \le w(u, v) + \operatorname{dist}^\beta(v \rightsquigarrow y)$：
      $$
      \begin{aligned}
        \varphi(v) &= \min(0, \max(\operatorname{dist}^\beta(x \rightsquigarrow v), -\operatorname{dist}^\beta(v \rightsquigarrow y))) \\
        &\le \min(0, \max(\operatorname{dist}^\beta(x \rightsquigarrow u) + w(u, v), -\operatorname{dist}^\beta(u \rightsquigarrow y) + w(u, v))) \\
        &= \min(-w(u, v), \max(\operatorname{dist}^\beta(x \rightsquigarrow u), -\operatorname{dist}^\beta(u \rightsquigarrow y))) + w(u, v) \\
        &\le \min(0, \max(\operatorname{dist}^\beta(x \rightsquigarrow u), -\operatorname{dist}^\beta(u \rightsquigarrow y))) + w(u, v) \\
        &= \varphi(u) + w(u, v) \text{。}
      \end{aligned}
      $$
  - $\varphi(u) = 0$ 对所有 $u \in U$。
    - 因为 $\operatorname{dist}^\beta(u \rightsquigarrow y) < 0$，故取负后 $> 0$，取 $\max$ 后仍 $> 0$，再取 $\min$ 后 $= 0$。
  - 对“大部分”其他点，$\varphi(v) \le \min(0, \operatorname{dist}^\beta(x \rightsquigarrow v))$。
    - 若 $v$ **不** $\beta$ 介于 $x \rightsquigarrow y$ 之间，有 $\operatorname{dist}^\beta(x \rightsquigarrow v) + \operatorname{dist}^\beta(v \rightsquigarrow y) \ge 0$。
    - 也即 $\operatorname{dist}^\beta(x \rightsquigarrow v) \ge -\operatorname{dist}^\beta(v \rightsquigarrow y)$。
    - 故 $\varphi(v) = \min(0, \operatorname{dist}^\beta(x \rightsquigarrow v))$。
    - 即只有 $1 / \tau$ 比例的点不满足这个条件。
- 现考虑 $R_\varphi^{\beta - 1}(U)$。对“大部分”其他点 $v$，有 $\operatorname{dist}_\varphi^{\beta - 1}(U \rightsquigarrow v) > 0$：
  - 根据上述，有 $\varphi(v) \le \min(0, \operatorname{dist}^\beta(x \rightsquigarrow v))$。
  - 又对任意 $u \in U$ 有 $\operatorname{dist}^\beta(x \rightsquigarrow v) \le \operatorname{dist}^1(x \rightsquigarrow u) + \operatorname{dist}^{\beta - 1}(u \rightsquigarrow v)$。
  - 然而，根据负夹心面包的定义，有 $\operatorname{dist}^1(x \rightsquigarrow u) < 0$，故 $\operatorname{dist}^\beta(x \rightsquigarrow v) < \operatorname{dist}^{\beta - 1}(u \rightsquigarrow v)$。
  - 然后是
    $$
    \begin{aligned}
      \operatorname{dist}_\varphi^{\beta - 1}(u \rightsquigarrow v) &= \operatorname{dist}^{\beta - 1}(u \rightsquigarrow v) + \varphi(u) - \varphi(v) \\
      &> \operatorname{dist}^\beta(x \rightsquigarrow v) + 0 - \min(0, \operatorname{dist}^\beta(x \rightsquigarrow v)) \\
      &= \max(0, \operatorname{dist}^\beta(x \rightsquigarrow v)) \\
      &\ge 0 \text{。}
    \end{aligned}
    $$
  - 故对“大部分”其他点 $v$，有 $\operatorname{dist}_\varphi^{\beta - 1}(U \rightsquigarrow v) > 0$。
...
- 设 $\hat k = \lvert U_0 \rvert$。再给定一个整数参数 $\rho \in [1, \hat k]$。有如下算法存在：
  - 在期望 $O((m + n \log n) \log n)$ 的时间内，得到以下三种结果之一：
    1. 汇报图中有负环；
    2. 找到一个大小为 $\Omega(\hat k / \rho)$ 的点集 $U \subseteq U_0$ 和一个点 $y \subseteq U_0$，满足 $\operatorname{dist}^1(u \rightsquigarrow y) < 0$ 对所有 $u \in U$；
...
- 不难看出，有了上述算法，并令 $U_0$ 为负点的一个子集，相当于每次可以找到“一半”的负夹心面包。所以有必要运行两趟上述算法，平衡指数可得：
  ...
  - 第一遍，令 $U_0 = V^-$，$\rho = k^{1/3}$：
    ...
    - 要么得到一个大小为 $\Omega(k^{2/3})$ 的负点集 $U_1$ 和一个点 $y$，满足 $\operatorname{dist}^1(u \rightsquigarrow y) < 0$ 对所有 $u \in U_1$。
    - 若得到 $I_1$，则算法结束。
  - 第二遍，令 $U_0 = U_1$，$\rho = k^{1/3}$，但在反向图上运行：
    ...
    - 要么得到一个 $1$ 跳无关点集 $I_2$，大小为 $\Omega(k^{1/3})$：
      ...
      - 在反向图上，$1$ 跳无关的描述为：对任意 $x, y \in I_2$ 有 $(\operatorname{dist}^1)^T(x \rightsquigarrow y) \ge 0$，注意上标 $T$ 表示反向图。
      - 在原图中，这就等价于 $\operatorname{dist}^1(y \rightsquigarrow x) \ge 0$。
      - 根据对称性可知，$I_2$ 是原图上的 $1$ 跳无关负点集。
    - 要么得到一个大小为 $\Omega(k^{1/3})$ 的点集 $U_2 \subseteq U_1$ 和一个点 $x$，满足 $(\operatorname{dist}^1)^T(u \rightsquigarrow x) < 0$ 对所有 $u \in U_2$：
      - 同样地，这里的 $U_2$ 不是反向图上的负点集，但确实是原图上的负点集。
      - 在原图中，$(\operatorname{dist}^1)^T(u \rightsquigarrow x) < 0$ 就等价于 $\operatorname{dist}^1(x \rightsquigarrow u) < 0$。
      - 结合 $\operatorname{dist}^1(u \rightsquigarrow y) < 0$ 对所有 $u \in U_2 \subseteq U_1$，可知 $(x, U_2, y)$ 是一个负夹心面包。
- 接下来我们将展示如何实现上述算法。
- 首先令记号 $C(U_0, v) = \lvert \{ u \in U_0 : \operatorname{dist}^1(u \rightsquigarrow v) < 0 \} \rvert$ 表示 $U_0$ 中负 $1$ 跳可达 $v$ 的点数。
...
- 得到了 $H, L$ 两个集合后（假设确实满足所有重点都在 $H$ 中、所有轻点都在 $L$ 中），我们分类讨论：
  - 若 $H \ne \varnothing$，即存在非轻点：
    ...
    - 令 $U = \{ u \in U_0 : \operatorname{dist}^1(u \rightsquigarrow y) < 0 \}$。
  ...
  - 若 $H = \varnothing$，即不存在重点：
    ...
    - $I'$ 中的一个点 $v$ **被其他点删去**的概率为：
      - 注：这里**被其他点删去**指的是 $v \in R^1(u)$ 对某个 $u \in I'$ 且 $u \ne v$。
        - 相对地，$v$ 可能被自己删去，即 $v \in R^1(v)$，即 $\operatorname{dist}^1(v \rightsquigarrow v) < 0$，然而这意味着存在负环。
    ...
    - 故进行 $\Theta(\log n)$ 次（具体地，$\lceil c' \ln n \rceil$ 次）对 $I'$ 的采样，就能将找到满足大小 $\ge \rho / 16$ 的 $1$ 跳无关的 $I$ 的概率提升到 $1 - 1 / \operatorname{poly}(n)$。
...
- 对于 $r$ 遥远负边，构造了一类称作负夹心面包的结构：
  ...
  - 具体来说，如果 $\operatorname{BW}^\beta(u \rightsquigarrow v) \le n / \tau$，且找到了一个负夹心面包 $(x, U, y)$，则经过一个合适的势函数后，$U$ 就是一个 $\min(\tau, \beta - 1)$ 遥远负边集。
  - 这个势函数在 $O(\beta (m + n \log n))$ 时间内可以计算出来。
- 我们先想办法降低图的间性：
  - 存在一个随机算法，在 $O(\beta \tau (m + n \log n) \log n + \tau^2 n \log^2 n)$ 的时间内，以 $1 - 1 / \operatorname{poly}(n)$ 的概率找到一个势函数，使得新图中每对点的 $\beta$ 间性都 $\le n / \tau$。
...
- 设 $X_1, \ldots, X_n$ 独立同分布 Bernoulli 随机变量，且 $\operatorname{E}[X_1] = \mu$。
- 则，对 $\varepsilon > 0$：
